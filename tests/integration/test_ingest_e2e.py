...
def test_e2e_ingestion_flow(
    ...
    mock_gh_repo.get.side_effect = lambda k: {"name": "TestRepo", "description": "Desc", "html_url": "http://gh.com/repo", "topics": ["test"]}.get(k)
    ...
    mock_gh_instance.fetch_repositories = AsyncMock(return_value=[
        Project(title="GH Project", description="Desc", repo_url="http://gh.com/repo", tags=["test"], source_platform="github", is_manual=False)
    ])
    
    mock_med_instance = mock_medium.return_value
    mock_med_instance.fetch_posts = AsyncMock(return_value=[
        Blog(title="Med Post", summary="Sum", date="2026-01-01", platform="Medium", url="http://med.com/post", source_platform="medium_rss", is_manual=False)
    ])
    
    mock_dev_instance = mock_devto.return_value
    mock_dev_instance.fetch_posts = AsyncMock(return_value=[
        Blog(title="Dev Post", summary="Sum", date="2026-01-02", platform="Dev.to", url="http://dev.to/post", source_platform="devto_api", is_manual=False)
    ])
    ...
    # Invoke CLI
    result = runner.invoke(ingest_module.app, [
        "--github-user", "user",
        "--medium-user", "user",
        "--devto-user", "user"
    ])
